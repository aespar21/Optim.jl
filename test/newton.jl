let
    ...
    d = TwiceDifferentiableFunction(f_1, g!_1, h!_1, [0.0])

    # Need to specify autodiff!
    @test_throws ArgumentError Optim.optimize(DifferentiableFunction(f_1, g!_1, [0.0]), [0.0], Newton())
    Optim.optimize(DifferentiableFunction(f_1, g!_1, [0.0]), [0.0], Newton(), Optim.Options(autodiff = true))
    ...
    for (name, prob) in Optim.UnconstrainedProblems.examples
    	if prob.istwicedifferentiable
    		ddf = TwiceDifferentiableFunction(prob.f, prob.g!,prob.h!, prob.initial_x)
