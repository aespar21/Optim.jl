...
```julia
results = optimize(OnceDifferentiable(f, g!, initial_x), initial_x, l, u, Fminbox(); optimizer = GradientDescent, iterations = 2)
```
In contrast, the following sets the maximum number of iterations for each `ConjugateGradient` optimization to 2
```julia
results = Optim.optimize(OnceDifferentiable(f, initial_x), initial_x, lower, upper, Fminbox(); optimizer = GradientDescent, optimizer_o = Optim.Options(iterations = 2))
```
